...
for (const mode of ['admin', 'root']) test(`DirectAdmin ${mode}: connect, select and queue a server import`, async ({ page }) => {
  ...
  await expect(page.getByText('Queued: customer (#22).', { exact: false })).toBeVisible()
  ...
  await expect(page.getByLabel(mode === 'root' ? 'Root SSH password' : 'Admin password or login key', { exact: true })).toHaveValue('')
})
