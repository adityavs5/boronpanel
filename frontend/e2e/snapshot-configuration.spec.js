...
for (const skin of ['evolution','paper-lantern']) for (const theme of ['light','dark']) {
  test(`${skin} ${theme}: scheduled-task restore and undo`, async ({ page }) => {
    ...
    await page.route('**/api/**',async route=>{
      ...
      else if(path.endsWith('/configuration'))data={cron_available:true,managed_jobs:1}
      else if(path.endsWith('/runs/1/restore')){
        restored=route.request().postDataJSON()
        history=[{id:10,status:'completed',selection:{kind:'config',config_sections:['cron']},safety_snapshot_id:'b'.repeat(64),progress_message:'Scheduled tasks restored',started_at:'2026-09-14T00:00:00Z'}]
    ...
    await detail.getByRole('button',{name:'Restore scheduled tasks',exact:true}).click()
    const form=detail.getByRole('form',{name:'Scheduled-task restore'})
    await expect(form.getByText(/complete crontab/)).toBeVisible()
    await expect(form.getByRole('button',{name:'Restore scheduled tasks now'})).toBeDisabled()
    await form.getByRole('textbox',{name:'Confirm account username',exact:true}).fill('alpha')
    await page.setViewportSize({width:390,height:844})
    await form.getByRole('button',{name:'Restore scheduled tasks now'}).click()
    expect(restored).toEqual({kind:'config',config_sections:['cron'],confirmation:'alpha'})
    await detail.getByRole('button',{name:'Recover previous scheduled tasks',exact:true}).click()
    const undo=page.getByRole('dialog').filter({has:page.getByRole('heading',{name:'Recover previous scheduled tasks?',exact:true})})
    await expect(undo.getByRole('button',{name:'Recover previous scheduled tasks',exact:true})).toBeDisabled()
    await undo.getByRole('textbox').fill('alpha')
    await undo.getByRole('button',{name:'Recover previous scheduled tasks',exact:true}).click()
