...
for (const skin of ['evolution','paper-lantern']) for (const theme of ['light','dark']) for (const section of ['cron', 'php']) {
  const label = section === 'php' ? 'PHP settings' : 'scheduled tasks'
  test(`${skin} ${theme}: ${label} restore and undo`, async ({ page }) => {
    ...
    await page.route('**/api/**',async route=>{
      ...
      else if(path.endsWith('/configuration'))data={cron_available:true,managed_jobs:1,php_available:true,php_sites:2,php_default_version:'8.3'}
      else if(path.endsWith('/runs/1/restore')){
        restored=route.request().postDataJSON()
        history=[{id:10,status:'completed',selection:{kind:'config',config_sections:[section]},safety_snapshot_id:'b'.repeat(64),progress_message:'Scheduled tasks restored',started_at:'2026-09-14T00:00:00Z'}]
    ...
    const form=detail.getByRole('form',{name:section === 'php' ? 'PHP settings restore' : 'Scheduled-task restore'})
    await expect(form.getByText(section === 'php' ? /Administrator function restrictions are kept/ : /complete crontab/)).toBeVisible()
    ...
    expect(restored).toEqual({kind:'config',config_sections:[section],confirmation:'alpha'})
