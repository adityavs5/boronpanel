...
for(const skin of ['evolution','paper-lantern']) for(const mode of ['light','dark']) {
 test(`${skin} ${mode}: select snapshot databases, restore and recover`,async({page},info)=>{
  ...
  await page.route('**/api/**',async route=>{
   ...
   else if(p.endsWith('/databases'))data={databases:[{name:'alpha_wp',size:1024,available:true},{name:'alpha_old',size:512,available:false,reason:'This database is no longer registered to this account.'}]}
  ...
  expect(restoreRequest).toEqual({kind:'databases',confirmation:'alpha',databases:['alpha_wp']})
