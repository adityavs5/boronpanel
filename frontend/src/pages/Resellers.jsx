...
const blankPlan = { name: '', max_accounts: 10, max_total_disk_mb: 102400, account_quota_soft_mb: 4096, account_quota_hard_mb: 5120, account_cpu_pct: 50, account_mem_mb: 1024, account_io_mb: 50, account_pids_max: 100, php_version: '8.3' }
...
function AdminResellers() {
  ...
  const planPayload = () => Object.fromEntries(Object.entries(plan).map(([key, value]) => [key, key === 'name' || key === 'php_version' ? value : Number(value)]))
  ...
  const openEditPlan = row => { setEditingPlan(row); setPlan(Object.fromEntries(Object.keys(blankPlan).map(key => [key, row[key]]))); setPlanOpen(true) }
