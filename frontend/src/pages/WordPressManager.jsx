...
function SiteManager({ site, domains, onClose, onChanged }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const changedKind=!dryRun&&command.startsWith('wp plugin ')&&!command.startsWith('wp plugin list')?'plugins':!dryRun&&command.startsWith('wp theme ')&&!command.startsWith('wp theme list')?'themes':null
      if(changedKind){
      ...
      if(dryRun&&['plugins','themes'].includes(tab)){
        if(!Array.isArray(data))throw new Error(`WordPress did not return an update list for ${tab}.`)
        const updates=new Map(data.map(item=>[item.name,item]))
        const installed=Array.isArray(result)?result:(extensionCache.get(extensionKey(site,tab))?.data||[])
        const merged=installed.map(item=>updates.has(item.name)?{...item,update:'available',update_version:updates.get(item.name).new_version||item.update_version}:item)
        extensionCache.set(extensionKey(site,tab),{data:merged,expires:Date.now()+EXTENSION_CACHE_MS})
        setResult(merged);setJob(null)
        return
      }
  ...
  return <Dialog open onOpenChange={open=>!open && !busy && onClose()}><DialogContent size="xl" className="wp-dialog"><DialogHeader><DialogTitle>{site.domain}{site.path ? '/'+site.path:''}</DialogTitle><DialogDescription>WordPress {site.wp_version || 'detected'} · {site.username}</DialogDescription></DialogHeader><DialogBody><div className="wp-tabs" role="tablist">{[['overview','Overview'],['plugins','Plugins'],['themes','Themes'],['backups','Backups'],['clone','Clone site'],['remove','Remove']].map(([id,label])=><button role="tab" aria-selected={tab===id} disabled={busy} key={id} onClick={()=>choose(id)}>{label}</button>)}</div><ErrorNotice error={error || polling.error}/><Progress job={job}/>
    ...
    {['plugins','themes'].includes(tab) && <div className="wp-extension-list"><div className="wp-section-heading"><h3>Installed {tab}</h3><div className="flex flex-wrap gap-2"><Button disabled={busy||!Array.isArray(result)} size="sm" variant="outline" onClick={()=>act(tab==='plugins'?'plugin_check_updates':'theme_check_updates',{},true)}>Check updates</Button><Button disabled={busy} size="sm" variant="outline" onClick={()=>choose(tab,true)}><RefreshCw size={14}/> Refresh installed</Button><Button disabled={busy} size="sm" onClick={()=>setConfirm({action:tab==='plugins'?'plugin_update':'theme_update',fields:{all:true},cli:true,label:`Update all ${tab}`,text:'A recent backup is recommended before updating.'})}>Update all</Button></div></div>{Array.isArray(result) ? result.map(item=><div className="wp-extension" key={item.name}><div><strong>{item.title || item.name}</strong><small>{item.version} · {item.status}{item.update_version?` · latest ${item.update_version}`:''}</small></div><span className="wp-badge">{item.update === 'available' ? 'Update available' : 'Installed'}</span><Button size="sm" variant="outline" disabled={busy || (tab==='themes' && item.status==='active')} onClick={()=>act(`${tab==='plugins'?'plugin':'theme'}_${item.status==='active' && tab==='plugins'?'deactivate':'activate'}`,{name:item.name},true)}>{item.status==='active' ? (tab==='plugins'?'Deactivate':'Active'):'Activate'}</Button></div>): !busy && <p className="wp-help">No extensions to display. Refresh to try loading the list again.</p>}</div>}
