...
export default function DirectAdminMigration({ onClose }) {
  ...
  return <section className="mb-6 rounded-panel border border-border bg-card p-5 space-y-5" aria-label="DirectAdmin server migration">
    ...
    {preview && <div className="space-y-4">
      ...
      <div className="flex gap-3"><Button variant="secondary" disabled={busy} onClick={() => setSelected(Object.fromEntries(preview.accounts.filter(row => row.available && !queued.some(job => job.username === row.username)).map(row => [row.username, row.username])))}>Select available</Button><Button variant="secondary" disabled={busy} onClick={() => setSelected({})}>Clear selection</Button></div>
      <div className="max-h-80 overflow-auto rounded-btn border border-border"><table className="w-full text-left"><thead className="bg-muted"><tr><th className="p-3">Source account</th><th className="p-3">New Boron username</th></tr></thead><tbody>{preview.accounts.map(row => <tr key={row.username} className="border-t border-border"><td className="p-3"><label className="flex items-center gap-2"><input type="checkbox" disabled={busy || queued.some(job => job.username === row.username)} checked={Object.hasOwn(selected, row.username)} onChange={event => setSelected(current => { const next = { ...current }; if (event.target.checked) next[row.username] = row.available ? row.username : ''; else delete next[row.username]; return next })} />{row.username}</label>{!row.available && <span className="text-sm text-muted-foreground">Already exists in Boron; choose a different name.</span>}</td><td className="p-3">{Object.hasOwn(selected, row.username) && <Input aria-label={`Destination username for ${row.username}`} disabled={busy} value={selected[row.username]} onChange={event => setSelected(current => ({ ...current, [row.username]: event.target.value }))} />}</td></tr>)}</tbody></table>{!preview.accounts.length && <p className="p-4">No source accounts found.</p>}</div>
    ...
    {!!queued.length && <p role="status">Queued: {queued.map(job => `${job.username} (#${job.id})`).join(', ')}. Progress and individual failures appear in the migration history below.</p>}
  </section>
}
