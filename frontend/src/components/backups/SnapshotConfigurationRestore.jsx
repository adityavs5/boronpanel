    ...
    {open && <form aria-label={dns ? 'DNS records restore' : php ? 'PHP settings restore' : 'Scheduled-task restore'} className="space-y-4 rounded-btn border border-border bg-muted/20 p-4" onSubmit={event => { event.preventDefault(); if (confirmation === username && available(catalog.data) && !catalog.isFetching && !catalog.error && !restore.isPending && (!dns || zones.length > 0)) restore.mutate() }}>
      ...
      {dns && !!catalog.data?.dns_zones?.length && <fieldset disabled={restore.isPending} className="space-y-2">
        ...
        {catalog.data.dns_zones.map(zone => <label key={zone.zone} className="flex items-start gap-3 rounded-btn border border-border bg-background p-3">
          ...
          <span className="min-w-0"><span className="block break-all text-sm font-medium">{zone.zone}</span><span className="text-xs text-muted-foreground">{zone.available ? `${zone.record_count} record sets` : zone.reason || 'Unavailable for recovery'}</span></span>
        </label>)}
      </fieldset>}
      {available(catalog.data) && <>
        ...
        <Button type="submit" loading={restore.isPending} disabled={confirmation !== username || catalog.isFetching || !!catalog.error || (dns && !zones.length)}>Restore {label} now</Button>
      </>}
    </form>}
  </section>
}
