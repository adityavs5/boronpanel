...
export function SnapshotRestoreHistory({ username }) {
  ...
  return <section className="mt-5 space-y-3"><h3 className="text-sm font-semibold">Restore history</h3><DataTable loading={history.isLoading} error={history.error} onRetry={history.refetch} data={history.data?.restores} pageSize={5} emptyTitle="No restores yet" columns={[
    ...
    {key:'progress_message',header:'Progress',render:r=><div className="text-sm"><p>{r.progress_message}</p>{r.summary?.safety_snapshot_expired&&<p className="text-muted-foreground">The previous-version copy expired under this backup job’s retention policy.</p>}{r.error&&<p className="text-danger">{r.error}</p>}{r.status==='failed'&&r.safety_snapshot_id&&<p className="mt-1 text-muted-foreground">{r.selection?.kind==='mail'?'The previous mail copy is retained. Contact your administrator for recovery assistance.':'Some selected data may have changed. Use the recovery copy to recover the previous version.'}</p>}</div>},
    {key:'actions',header:'Recovery',render:r=>r.safety_snapshot_id&&!['pending','running'].includes(r.status)?r.selection?.kind==='mail'?<span className="text-sm text-muted-foreground">Previous mail copy saved</span>:<Button size="sm" variant="secondary" onClick={()=>setUndo(r)}><RotateCcw className="h-3.5 w-3.5"/>{r.selection?.kind==='databases'?'Recover previous databases':'Recover previous files'}</Button>:null},
