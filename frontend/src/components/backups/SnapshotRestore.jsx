...
export function SnapshotRestoreHistory({ username }) {
  ...
  const reversal=useMutation({mutationFn:()=>post(`${base}/restores/${undo.id}/undo`,{confirmation:username,...(pausesMail(undo)?{mail_pause_acknowledged:true}:{})}),onSuccess:()=>{toast.success('Previous-version recovery queued');setUndo(null);qc.invalidateQueries({queryKey:['snapshot-restores',username]})},onError:e=>toast.error('Could not recover previous version',e.message)})
  ...
  return <section className="mt-5 space-y-3"><h3 className="text-sm font-semibold">Restore history</h3><DataTable loading={history.isLoading} error={history.error} onRetry={history.refetch} data={history.data?.restores} pageSize={5} emptyTitle="No restores yet" columns={[
    ...
    {key:'actions',header:'Recovery',render:r=>r.safety_snapshot_id&&!['pending','running'].includes(r.status)&&(!isRoutingRecovery(r)||(r.status==='completed'&&r.summary?.routing_finalized))?<Button size="sm" variant="secondary" onClick={()=>setUndo(r)}><RotateCcw className="h-3.5 w-3.5"/>{isRoutingRecovery(r)?'Recover previous email settings':isDnsRecovery(r)?'Recover previous DNS records':isPhpRecovery(r)?'Recover previous PHP settings':r.selection?.kind==='config'?'Recover previous scheduled tasks':r.selection?.kind==='mail'?'Recover previous mail':r.selection?.kind==='databases'?'Recover previous databases':'Recover previous files'}</Button>:null},
  ]}/><ConfirmDialog open={!!undo} onOpenChange={open=>!open&&setUndo(null)} title={isRoutingRecovery(undo)?'Recover previous email settings?':isDnsRecovery(undo)?'Recover previous DNS records?':isPhpRecovery(undo)?'Recover previous PHP settings?':undo?.selection?.kind==='config'?'Recover previous scheduled tasks?':undo?.selection?.kind==='mail'?'Recover previous mail?':undo?.selection?.kind==='databases'?'Recover previous database versions?':'Recover previous file versions?'} description={isRoutingRecovery(undo)?`Restore forwarders, catch-all addresses and automatic replies saved before this operation for ${(undo.selection.mail_domains || []).join(', ')}. Current settings are saved in another encrypted recovery copy first. Mailbox messages, passwords and quotas are kept. Mail access across this server pauses briefly and incoming mail is queued.`:isDnsRecovery(undo)?`Restore previous customer DNS records for ${(undo.selection.dns_zones || []).join(', ')}. Current records are saved in another encrypted recovery copy first. These changes can affect websites and email. Server-managed nameservers and DNSSEC settings are kept.`:isPhpRecovery(undo)?'Restore the PHP versions, limits and extension choices saved before this operation. Current PHP settings are saved in another encrypted recovery copy first. Administrator function restrictions are kept.':undo?.selection?.kind==='config'?'Replace the complete crontab with the settings saved before this restore. Current scheduled tasks and environment settings are saved in another encrypted recovery copy first.':undo?.selection?.kind==='mail'?'Replace messages with the versions saved before this restore. Current messages are saved in another encrypted safety copy. Mailboxes must still exist; their current passwords and settings are kept. Mail access across this server pauses briefly and incoming mail is queued.':undo?.selection?.kind==='databases'?'Import the database versions saved immediately before this operation. Current tables will be replaced, including removal of tables created since that recovery copy. Newly recreated databases are retained.':'Restore the file versions saved immediately before this operation. Newer changes to those files will be replaced. Files that had no previous version are retained.'} confirmationText={username} acknowledgementLabel={pausesMail(undo)?'I understand that mail access will be interrupted briefly.':undefined} confirmLabel={isRoutingRecovery(undo)?'Recover previous email settings':isDnsRecovery(undo)?'Recover previous DNS records':isPhpRecovery(undo)?'Recover previous PHP settings':undo?.selection?.kind==='config'?'Recover previous scheduled tasks':undo?.selection?.kind==='mail'?'Recover previous mail':undo?.selection?.kind==='databases'?'Recover previous databases':'Recover previous files'} loading={reversal.isPending} onConfirm={()=>reversal.mutate()}/></section>
}
