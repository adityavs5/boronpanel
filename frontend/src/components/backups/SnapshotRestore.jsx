...
export function SnapshotDatabaseRestore({ username, run }) {
  ...
  return <section className="space-y-3 border-t border-border pt-4">
    ...
    {open&&<form aria-label="Database restore" className="space-y-4 rounded-btn border border-border bg-muted/20 p-4" onSubmit={e=>{e.preventDefault();restore.mutate()}}>
      <div><h3 className="font-semibold">Choose databases to restore</h3><p className="mt-1 text-sm text-muted-foreground">Backed-up tables replace their current versions. Tables added after this backup stay in place. Database users and passwords remain unchanged.</p></div>
...
export function SnapshotRestoreHistory({ username }) {
  ...
  return <section className="mt-5 space-y-3"><h3 className="text-sm font-semibold">Restore history</h3><DataTable loading={history.isLoading} error={history.error} onRetry={history.refetch} data={history.data?.restores} pageSize={5} emptyTitle="No restores yet" columns={[
    ...
    {key:'progress_message',header:'Progress',render:r=><div className="text-sm"><p>{r.progress_message}</p>{r.error&&<p className="text-danger">{r.error}</p>}{r.status==='failed'&&r.safety_snapshot_id&&<p className="mt-1 text-muted-foreground">Some selected data may have changed. Use the recovery copy to recover the previous version.</p>}</div>},
  ...
  ]}/><ConfirmDialog open={!!undo} onOpenChange={open=>!open&&setUndo(null)} title={undo?.selection?.kind==='databases'?'Recover previous database versions?':'Recover previous file versions?'} description={undo?.selection?.kind==='databases'?'Import the database versions saved immediately before this operation. Newer changes to captured tables will be replaced. Tables added later stay in place.':'Restore the file versions saved immediately before this operation. Newer changes to those files will be replaced. Files that had no previous version are retained.'} confirmationText={username} confirmLabel={undo?.selection?.kind==='databases'?'Recover previous databases':'Recover previous files'} loading={reversal.isPending} onConfirm={()=>reversal.mutate()}/></section>
}
