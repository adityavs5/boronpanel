...
export function SnapshotDatabaseRestore({ username, run }) {
  ...
  return <section className="space-y-3 border-t border-border pt-4">
    ...
    {open&&<form aria-label="Database restore" className="space-y-4 rounded-btn border border-border bg-muted/20 p-4" onSubmit={e=>{e.preventDefault();restore.mutate()}}>
      <div><h3 className="font-semibold">Choose databases to restore</h3><p className="mt-1 text-sm text-muted-foreground">All current tables in the selected databases will be replaced by the backup, including removal of tables created afterward. Existing database users and passwords remain unchanged. Deleted databases with recovery metadata can be recreated using their backed-up credentials.</p></div>
      <p className="flex gap-2 text-sm text-muted-foreground"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0"/>A recovery copy of each existing selected database is saved first. Newly recreated databases have no previous version. Pause writes to your website while restoring to avoid losing new changes.</p>
...
const isPhpRecovery = row => row?.selection?.kind === 'config' && row.selection.config_sections?.includes('php')
...
export function SnapshotRestoreHistory({ username }) {
  ...
  const reversal=useMutation({mutationFn:()=>post(`${base}/restores/${undo.id}/undo`,{confirmation:username,...(undo.selection?.kind==='mail'?{mail_pause_acknowledged:true}:{})}),onSuccess:()=>{toast.success('Previous-version recovery queued');setUndo(null);qc.invalidateQueries({queryKey:['snapshot-restores',username]})},onError:e=>toast.error('Could not recover previous version',e.message)})
  ...
  return <section className="mt-5 space-y-3"><h3 className="text-sm font-semibold">Restore history</h3><DataTable loading={history.isLoading} error={history.error} onRetry={history.refetch} data={history.data?.restores} pageSize={5} emptyTitle="No restores yet" columns={[
    ...
    {key:'kind',header:'Contents',render:r=>isDnsRecovery(r)?'DNS records':isPhpRecovery(r)?'PHP settings':({databases:'Databases',mail:'Mailboxes',files:'Files',config:'Scheduled tasks'})[r.selection?.kind]||'Files'},
    ...
    {key:'progress_message',header:'Progress',render:r=><div className="text-sm"><p>{r.progress_message}</p>{r.summary?.safety_snapshot_expired&&<p className="text-muted-foreground">The previous-version copy expired under this backup job’s retention policy.</p>}{r.error&&<p className="text-danger">{r.error}</p>}{r.status==='failed'&&r.safety_snapshot_id&&<p className="mt-1 text-muted-foreground">{r.selection?.kind==='mail'?'The previous mail copy is retained and can be recovered while these mailboxes still exist.':'Some selected data may have changed. Use the recovery copy to recover the previous version.'}</p>}</div>},
    {key:'actions',header:'Recovery',render:r=>r.safety_snapshot_id&&!['pending','running'].includes(r.status)?<Button size="sm" variant="secondary" onClick={()=>setUndo(r)}><RotateCcw className="h-3.5 w-3.5"/>{isDnsRecovery(r)?'Recover previous DNS records':isPhpRecovery(r)?'Recover previous PHP settings':r.selection?.kind==='config'?'Recover previous scheduled tasks':r.selection?.kind==='mail'?'Recover previous mail':r.selection?.kind==='databases'?'Recover previous databases':'Recover previous files'}</Button>:null},
  ]}/><ConfirmDialog open={!!undo} onOpenChange={open=>!open&&setUndo(null)} title={isDnsRecovery(undo)?'Recover previous DNS records?':isPhpRecovery(undo)?'Recover previous PHP settings?':undo?.selection?.kind==='config'?'Recover previous scheduled tasks?':undo?.selection?.kind==='mail'?'Recover previous mail?':undo?.selection?.kind==='databases'?'Recover previous database versions?':'Recover previous file versions?'} description={isDnsRecovery(undo)?`Restore previous customer DNS records for ${(undo.selection.dns_zones || []).join(', ')}. Current records are saved in another encrypted recovery copy first. These changes can affect websites and email. Server-managed nameservers and DNSSEC settings are kept.`:isPhpRecovery(undo)?'Restore the PHP versions, limits and extension choices saved before this operation. Current PHP settings are saved in another encrypted recovery copy first. Administrator function restrictions are kept.':undo?.selection?.kind==='config'?'Replace the complete crontab with the settings saved before this restore. Current scheduled tasks and environment settings are saved in another encrypted recovery copy first.':undo?.selection?.kind==='mail'?'Replace messages with the versions saved before this restore. Current messages are saved in another encrypted safety copy. Mailboxes must still exist; their current passwords and settings are kept. Mail access across this server pauses briefly and incoming mail is queued.':undo?.selection?.kind==='databases'?'Import the database versions saved immediately before this operation. Current tables will be replaced, including removal of tables created since that recovery copy. Newly recreated databases are retained.':'Restore the file versions saved immediately before this operation. Newer changes to those files will be replaced. Files that had no previous version are retained.'} confirmationText={username} acknowledgementLabel={undo?.selection?.kind==='mail'?'I understand that mail access will be interrupted briefly.':undefined} confirmLabel={isDnsRecovery(undo)?'Recover previous DNS records':isPhpRecovery(undo)?'Recover previous PHP settings':undo?.selection?.kind==='config'?'Recover previous scheduled tasks':undo?.selection?.kind==='mail'?'Recover previous mail':undo?.selection?.kind==='databases'?'Recover previous databases':'Recover previous files'} loading={reversal.isPending} onConfirm={()=>reversal.mutate()}/></section>
}
