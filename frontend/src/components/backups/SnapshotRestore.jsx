...
export function SnapshotRestoreHistory({ username }) {
  ...
  return <section className="mt-5 space-y-3"><h3 className="text-sm font-semibold">Restore history</h3><DataTable loading={history.isLoading} error={history.error} onRetry={history.refetch} data={history.data?.restores} pageSize={5} emptyTitle="No restores yet" columns={[
    ...
    {key:'kind',header:'Contents',render:r=>({databases:'Databases',mail:'Mailboxes',files:'Files',config:'Scheduled tasks'})[r.selection?.kind]||'Files'},
    ...
    {key:'actions',header:'Recovery',render:r=>r.safety_snapshot_id&&!['pending','running'].includes(r.status)?<Button size="sm" variant="secondary" onClick={()=>setUndo(r)}><RotateCcw className="h-3.5 w-3.5"/>{r.selection?.kind==='config'?'Recover previous scheduled tasks':r.selection?.kind==='mail'?'Recover previous mail':r.selection?.kind==='databases'?'Recover previous databases':'Recover previous files'}</Button>:null},
  ]}/><ConfirmDialog open={!!undo} onOpenChange={open=>!open&&setUndo(null)} title={undo?.selection?.kind==='config'?'Recover previous scheduled tasks?':undo?.selection?.kind==='mail'?'Recover previous mail?':undo?.selection?.kind==='databases'?'Recover previous database versions?':'Recover previous file versions?'} description={undo?.selection?.kind==='config'?'Replace the complete crontab with the settings saved before this restore. Current scheduled tasks and environment settings are saved in another encrypted recovery copy first.':undo?.selection?.kind==='mail'?'Replace messages with the versions saved before this restore. Current messages are saved in another encrypted safety copy. Mailboxes must still exist; their current passwords and settings are kept. Mail access across this server pauses briefly and incoming mail is queued.':undo?.selection?.kind==='databases'?'Import the database versions saved immediately before this operation. Current tables will be replaced, including removal of tables created since that recovery copy. Newly recreated databases are retained.':'Restore the file versions saved immediately before this operation. Newer changes to those files will be replaced. Files that had no previous version are retained.'} confirmationText={username} acknowledgementLabel={undo?.selection?.kind==='mail'?'I understand that mail access will be interrupted briefly.':undefined} confirmLabel={undo?.selection?.kind==='config'?'Recover previous scheduled tasks':undo?.selection?.kind==='mail'?'Recover previous mail':undo?.selection?.kind==='databases'?'Recover previous databases':'Recover previous files'} loading={reversal.isPending} onConfirm={()=>reversal.mutate()}/></section>
}
