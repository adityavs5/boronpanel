...
operations. Ten deliberately public routes are marked `reviewed-public` and
locked by `tests/test_public_route_inventory.py`; the other 832 entries remain
...
| Fresh LiteSpeed repository setup | Bundled public key verifies the cached signed Noble Release; APT source uses `signed-by` over HTTPS instead of root-executed remote setup script | Fresh installer source fixed; fresh VM install and existing global-key migration pending |
...
These are dependency observations, not
