...
## Self-update guard reload correction

Update job 6 passed its full live preflight and backup, downloaded and staged 1.2.0, and built its isolated environment. It then stopped safely before switching `/opt/boron`: the staged mailbox guard activation observed Dovecot's transient reload substate and rejected it as though the service were unhealthy. The live symlink remained `/opt/boron-1.1.3`; API, provisioner and Dovecot remained active.

The staged guard and effective configuration both passed read-only verification. Reproduction isolated the failure to the health callback immediately after `doveadm reload`. The corrected callback preserves the strict initial running-service gate, waits up to 20 seconds only through active reload transitions, and still rolls back on a terminal or timed-out state. All 81 focused guard/updater tests passed (`/tmp/boron-update-guard-fix-tests.log`), followed by a successful real guard activation, reload and verification with a private configuration backup. The immutable published 1.2.0 release is retained; the correction will ship as 1.2.1 and requires the full release and self-update gates again.

## Corrected release and successful self-update

The real `scripts/release.sh 1.2.1` pipeline passed all **2,659 tests with seven warnings and no skips** in 2,800.56 seconds, built the frontend, staged 744 tracked files and self-verified its archive and SHA256 checksum. It published tag `v1.2.1` and the release assets to GitHub. An independent public download passed the published checksum and matched the local verified archive byte-for-byte.

Update job **7** then completed the actual protected 1.1.3-to-1.2.1 path. Its full live preflight passed, followed by online backup, public release download, checksum verification, extraction, isolated environment creation, migration and the corrected mailbox-guard activation. The detached finalizer switched `/opt/boron` to `/opt/boron-1.2.1`, restarted API and provisioner, and passed its API-plus-daemon health check. The job completed without rollback.

The authenticated updater reports current/latest version 1.2.1 with no update available. The post-update audit passed all service, synchronized-clock, trusted HTTPS, authenticated API, shared-port and idle-queue checks. The `/etc/boron/boron.toml` digest and WordPress installation, snapshot-run and backup-policy inventories match the saved pre-update baseline exactly. Both authoritative Cloudflare nameservers return `phpmyadmin.boron.sitecountry.com A 104.234.179.66`, and trusted HTTPS reaches `/boron_signon.php`. Private evidence is stored at `/root/boron-setup/release-1.2.1-after.json` with mode 0600. This closes the initial implementation and release gate; the queued product expansion remains active.
