...
The revised complete suite is running in `/tmp/boron-full-schema-final-tests.log`. Ubuntu's ShellCheck package was installed, and the previously skipped release static check now passes (`/tmp/boron-release-shellcheck-tests.log`, one test in 2.00 seconds). The complete suite remains the outstanding regression gate.
