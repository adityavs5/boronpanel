...
logger = logging.getLogger("forgehostd.fail2ban")
...
def _render_jail_conf(cf_ranges: list[str]) -> str:
    """Full jail.d/forgehost.conf: a [DEFAULT] block whose `ignoreip` never
    ...
    ignoreip = " ".join(_IGNOREIP_BASE + list(cf_ranges))
    default_block = (
        "# Managed by Forgehost (daemon/fail2ban.py). Do not edit by hand --\n"
...
def refresh_cloudflare_ignoreip() -> bool:
    """Rewrite jail.d/forgehost.conf with the current Cloudflare ranges and
