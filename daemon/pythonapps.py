...
def list_all_apps(params: dict | None = None) -> dict:
    ...
    with write_session() as session:
        rows = session.execute(
            select(PythonApp, Account.username)
            .join(Account, Account.id == PythonApp.account_id)
            .order_by(Account.username, PythonApp.name)
        ).all()
        apps = []
        for row, username in rows:
            ...
            apps.append({
                ...
                "port": row.port, "app_type": row.app_type,
