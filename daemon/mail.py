...
Dovecot's SQL passdb/userdb query boron_mail live -- routine mailbox/
domain CRUD here needs **no Postfix/Dovecot reload at all**, just an INSERT/
UPDATE/DELETE against boron_mail (a real operational difference from
...
boron_daemon (the same admin user from Phase d) is used for writes here;
boron_mailro (SELECT-only) is the credential embedded in Postfix's and
